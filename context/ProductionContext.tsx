...
export interface Order {
  ...
  can_fulfill?: boolean;
  missing_materials?: { material_id: string; needed: number; available: number }[];
  created_at: string;
}
...
interface ProductionContextType {
  ...
  createPurchaseOrder: (prId: string, supplier: string, deliveryDate: string) => void;
  ...
  updateInventory: (materialId: string, onHand: number, reserved: number) => void;
}
...
const getInitialOrders = (): Order[] => {
  ...
  return [
    ...
    },
    {
      ...
      can_fulfill: false,
      missing_materials: [
        { material_id: 'm2', needed: 400, available: 200 },
      ],
...
export function ProductionProvider({ children }: { children: ReactNode }) {
  ...
  const [purchaseRequests, setPurchaseRequests] = useState<PurchaseRequest[]>(getInitialPurchaseRequests());
  const [purchaseOrders, setPurchaseOrders] = useState<PurchaseOrder[]>(getInitialPurchaseOrders());
  const [productionSchedules, setProductionSchedules] = useState<ProductionSchedule[]>(getInitialProductionSchedules());

  const addOrder = (orderData: Omit<Order, 'id' | 'status' | 'created_at'>): string => {
  ...
  const checkOrderFulfillment = (orderId: string): boolean => {
    ...
    const missingMaterials: { material_id: string; needed: number; available: number }[] = [];
    let canFulfill = true;
    ...
    setOrders(prev => prev.map(o => 
      o.id === orderId 
        ? { ...o, can_fulfill: canFulfill, missing_materials: missingMaterials }
        : o
    ));
  ...
  const createPurchaseRequest = (orderId: string) => {
    const order = orders.find(o => o.id === orderId);
    if (!order || !order.missing_materials || order.missing_materials.length === 0) return;
  ...
  const createPurchaseOrder = (prId: string, supplier: string, deliveryDate: string) => {
...
//   const createPurchaseOrderDirect = (
//   materialId: string,
//   quantity: number,
//   supplier: string,
//   deliveryDate: string
...
//     // Không có PR nên đặt pr_id = null hoặc undefined
//     pr_id: '',
...
// };
  ...
  const scheduleProduction = (orderId: string) => {
    ...
    const newSchedule: ProductionSchedule = {
      ...
      end_date: new Date(deliveryDate.getTime() - 86400000).toISOString().split('T')[0], // 1 ngày trước giao
      status: 'scheduled',
    };
  ...
  const startProduction = (scheduleId: string) => {
    setProductionSchedules(prev => prev.map(s => 
    ...
    if (schedule) {
      setOrders(prev => prev.map(o => 
        o.id === schedule.order_id ? { ...o, status: 'in_production' as const } : o
      ));
    }
  };
  ...
  const completeProduction = (scheduleId: string) => {
    ...
    bomItems.forEach(bomItem => {
      ...
      setInventory(prev => prev.map(inv => 
        inv.material_id === bomItem.material_id
          ? { ...inv, on_hand: inv.on_hand - needed, reserved: inv.reserved - needed }
          : inv
      ));
    });

    setProductionSchedules(prev => prev.map(s => 
      s.id === scheduleId ? { ...s, status: 'completed' as const } : s
    ));
  ...
  const updateInventory = (materialId: string, onHand: number, reserved: number) => {
    setInventory(prev => prev.map(inv => 
      inv.material_id === materialId ? { ...inv, on_hand: onHand, reserved } : inv
    ));
  };
