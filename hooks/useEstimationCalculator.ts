...
import { useProductStandardization } from './useProductStandardization';
import { usePaperEstimation } from './usePaperEstimation';
import { useCostEstimation } from './useCostEstimation';
import { useProductionTime } from './useProductionTime';
...
export const useEstimationCalculator = (): UseEstimationCalculator => {
  const { getProductTypeCode, calculatePrintSize } = useProductStandardization();
  const { calculateNUp, calculateBaseSheets, calculateTotalWaste } = usePaperEstimation();
  const {
    calculatePrintArea,
  ...
  const { calculateProductionDays, calculateRushFee } = useProductionTime();
  ...
  const calculateAll = (
    ...
    const overheadCost = calculateOverheadCost(materialCost, config?.overhead_percent);
    ...
    if (processCosts && processes.length > 0) {
      processes.forEach(processCode => {
        ...
        if (processConfig) {
          ...
          if (qtyForProcess > 0) {
            totalProcessCost += qtyForProcess * (processConfig.unit_price || 0);
          }
        }
      });
    }
