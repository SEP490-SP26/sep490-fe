...
import { calculateProductionTime, getEstimatedFreeDate, mapToOrderEstimationResult } from "@/app/consultant/utils/consultant-logic";
import DesignUploadSection from "@/app/consultant/components/DesignUploadSection";
import ProductSpecsSection from "@/app/consultant/components/ProductSpecsSection";
import CustomerInfoSection from "@/app/consultant/components/CustomerInfoSection";
import EstimatesCard from "@/app/consultant/components/EstimatesCard";
import FactoryOrdersModal from "@/app/consultant/components/FactoryOrdersModal";
          ...
          <div className="mb-4">
            <Collapse
              ...
              items={[
                {
                  key: '1',
                  label: <strong className="text-gray-800 text-base">📖 Bảng hướng dẫn công thức tính toán tự động</strong>,
                  children: (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-lg text-sm text-gray-700">
                      <div>
                        <strong className="text-gray-900 text-sm">1. Số tờ giấy in:</strong>
                        <p className="mt-1.5 font-mono text-xs bg-white p-3 rounded border border-gray-200 text-gray-600">
                          <span className="block mb-1">Số tờ in = Làm tròn lên (Số lượng SP / Số con trên tờ in)</span>
                          <span className="block">Tổng số tờ cần xuất = Số tờ in + Bù hao in ấn + Bù hao gia công (Bế, Bồi, Cán...)</span>
                        </p>
                      </div>
                      <div>
                        <strong className="text-gray-900 text-sm">2. Tổng diện tích (m²):</strong>
                        <p className="mt-1.5 font-mono text-xs bg-white p-3 rounded border border-gray-200 text-gray-600">
                          Diện tích (m²) = Số tờ in × [ Chiều dài tờ in (mm) × Chiều rộng tờ in (mm) / 1.000.000 ]
                        </p>
                      </div>
                      <div>
                        <strong className="text-gray-900 text-sm">3. Chi phí mực in:</strong>
                        <p className="mt-1.5 font-mono text-xs bg-white p-3 rounded border border-gray-200 text-gray-600">
                          <span className="block mb-1">Khối lượng mực (kg) = Diện tích (m²) × Định mức tiêu hao mực (kg/m²)</span>
                          <span className="block">Chi phí mực = Khối lượng mực (kg) × Đơn giá mực (VNĐ/kg)</span>
                        </p>
                      </div>
                      <div>
                        <strong className="text-gray-900 text-sm">4. Chi phí phủ / cán màng / keo bồi:</strong>
                        <p className="mt-1.5 font-mono text-xs bg-white p-3 rounded border border-gray-200 text-gray-600">
                          <span className="block mb-1">Khối lượng vật tư (kg) = Diện tích (m²) × Định mức tiêu hao (kg/m²)</span>
                          <span className="block">Chi phí = Khối lượng vật tư (kg) × Đơn giá vật tư (VNĐ/kg)</span>
                        </p>
                      </div>
                      <div>
                        <strong className="text-gray-900 text-sm">5. Chi phí gia công:</strong>
                        <p className="mt-1.5 font-mono text-xs bg-white p-3 rounded border border-gray-200 text-gray-600">
                          Chi phí công đoạn = Số lượng (hoặc diện tích) × Đơn giá công đoạn
                        </p>
                      </div>
                      <div>
                        <strong className="text-gray-900 text-sm">6. Tổng giá thành:</strong>
                        <p className="mt-1.5 font-mono text-xs bg-white p-3 rounded border border-gray-200 text-gray-600">
                          <span className="block mb-1">Chi phí NVL = Chi phí giấy + Chi phí mực + Chi phí phủ + Chi phí cán + Chi phí keo bồi</span>
                          <span className="block mb-1">Giá thành cơ bản = Chi phí NVL + Tổng chi phí gia công + Chi phí kẽm + Chi phí khuôn + Chi phí thiết kế</span>
                          <span className="block font-medium text-blue-600">Tổng cuối cùng = (Giá thành cơ bản + Phụ thu gấp - Giảm giá) × (1 + %VAT)</span>
                        </p>
                      </div>
                    </div>
                  ),
