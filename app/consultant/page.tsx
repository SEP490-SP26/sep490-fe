...
import { FloatingInputNumberAntd } from "@/components/Input/FloatingInputNumber";
...
function ConsultantForm() {
  ...
  const [selectedProductTypeCode, setSelectedProductTypeCode] =
    useState<string>("");
  const [selectProductTypeId, setselectProductTypeId] = useState<number>();

  console.log("select Product code", selectedProductTypeCode);
  console.log("select Product id", selectProductTypeId);
  ...
  const [processCostBreakdown, setProcessCostBreakdown] =
    useState<ProcessCostBreakdownResponse | null>(null);
    ...
    const fetchPaperTypes = async () => {
      ...
      try {
        ...
        if (response?.paperTypes && Array.isArray(response.paperTypes)) {
          ...
          setPaperTypes(
            response.paperTypes.map((pt: { code: string; name: string }) => ({
              code: pt.code,
              name: pt.name,
            }))
          );
  ...
  */
  ...
  const calculatePaperEstimate = async () => {
    ...
    try {
      ...
      const response = await estimatesApi.estimatePaper({
        ...
        wave_type:
          productionProcessesStr.includes("BOI") && waveType ? waveType : "",
      });
  ...
  //   queryKey: ["product-tempalte"],
  //   queryFn: async () => {
  //     if (!selectProductTypeId) {
  //       throw new Error("Order ID is required");
  //     }
  //     const response = await productTypesApi.getProductTemplete(
  //       selectProductTypeId
  //     );
  //     return response;
  //   },
  ...
  // console.log('res product template: ',productTempalte)
  ...
  const calculateCostEstimate = async (
    ...
    try {
      ...
      const response = await estimatesApi.estimateCost({
        ...
        // wave_type: có giá trị khi BOI, rỗng khi không có BOI
        wave_type:
          productionProcesses.includes("BOI") && waveType ? waveType : "",
      });
      ...
      if (response) {
        ...
        try {
          const depositResponse = await estimatesApi.getDeposit(
            parseInt(orderId)
          );
        ...
        if (orderId) {
          ...
          if (isNegotiateMode) {
            ...
            if (!finalPrice || finalPrice <= 0) {
              message.warning(
                "Vui lòng nhập giá chốt hợp lệ trước khi gửi báo giá!"
              );
              setLoading(false);
              return;
            }
                    ...
                    <Col span={4}>
                      ...
                      >
                        <FloatingInputAntd
                          ...
                          disabled={!!orderId}
                          className={
                            orderId ? "bg-gray-50 text-end" : "text-end"
                          }
                        />
                      </Form.Item>
                    </Col>
                    ...
                    <Col span={12}>
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: "Vui lòng chọn loại form",
                          },
                        ]}
                      >
                        <Select
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Form.Item name="width" noStyle>
                          <FloatingInputAntd
                            ...
                            controls={false}
                            min={1}
                            // Quan trọng: thêm name để form submit
                            // name="radius" // hoặc field name tương ứng
                          />
                        </Form.Item>
                        <span className="text-gray-400">×</span>
                      ...
                      {({ getFieldValue }) => {
                        ...
                        return hasBOI ? (
                          <Form.Item
                            ...
                            label="Loại Sóng (Bồi)"
                            rules={[
                              { required: true, message: "Chọn loại sóng!" },
                            ]}
                            className="mb-1"
                          >
                            <Select
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="border-t pt-2 mt-2">
                            <div className="flex justify-between text-gray-700">
                              ...
                              <span className="font-semibold">
                                {(
                                  Math.round(
                                    costEstimate.cost.material_cost / 10
                                  ) * 10
                                ).toLocaleString("vi-VN")}{" "}
                                ₫
                              </span>
                            ...
                            <div className="flex justify-between text-gray-700">
                              ...
                              <span>
                                {(
                                  Math.round(
                                    costEstimate.cost.overhead_cost / 10
                                  ) * 10
                                ).toLocaleString("vi-VN")}{" "}
                                ₫
                              </span>
                          ...
                          <div className="flex justify-between font-medium">
                            ...
                            <span className="text-blue-700">
                              {(
                                Math.round(costEstimate.cost.base_cost / 10) *
                                10
                              ).toLocaleString("vi-VN")}{" "}
                              ₫
                            </span>
                          ...
                          <div className="border-t-2 border-blue-300 pt-3 mt-3">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="font-bold text-xl text-blue-700">
                                {(
                                  Math.round(
                                    costEstimate.cost.final_total_cost / 10
                                  ) * 10
                                ).toLocaleString("vi-VN")}{" "}
                                ₫
                              </span>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            message.error(
                              "Có lỗi khi cập nhật giá. Vui lòng thử lại."
                            );
                          }
                        }}
                      >
