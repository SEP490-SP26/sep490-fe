...
export default function Dashboard() {
  ...
  const scheduledOrders = orders.filter(o => o.status === 'scheduled' || o.status === 'in_production').length;
  const scheduledRate = totalOrders > 0 ? ((scheduledOrders / totalOrders) * 100).toFixed(0) : 0;
  ...
  const todaySchedules = productionSchedules.filter(s => {
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {recentOrders.length > 0 ? (
                ...
                <div key={order.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex-1">
                    ...
                    <div className="text-gray-500 text-sm">
                      Số lượng: {order.quantity} • Giao: {new Date(order.delivery_date).toLocaleDateString('vi-VN')}
                    </div>
                  </div>
              ...
              <div className="text-gray-400 text-center py-8">Chưa có đơn hàng</div>
            )}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {lowStockItems.length > 0 ? (
              lowStockItems.map(item => (
                <div key={item.material_id} className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                  <div className="flex-1">
                    <div className="text-gray-900">{item.material?.name}</div>
                    <div className="text-gray-500 text-sm">
                      Khả dụng: {item.on_hand - item.reserved} {item.material?.unit}
                    </div>
                  </div>
              ...
              <div className="text-gray-400 text-center py-8">Tồn kho ổn định</div>
            )}
          </div>
        </div>
              ...
              todaySchedules.map(schedule => (
                <div key={schedule.id} className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs ${
                    ...
                    {schedule.status === 'in_progress' ? 'Đang sản xuất' : 'Đã lên lịch'}
                  </span>
                </div>
              ))
            ) : (
              <div className="text-gray-400 text-center py-8">Không có lịch sản xuất hôm nay</div>
            )}
          </div>
        </div>
      </div>
